...
if (table_exists($pdo, 'meta_sync_runs')) {
    ...
    $auditMetaSummary = array_merge($auditMetaSummary, array_map('intval', $stmt->fetch(PDO::FETCH_ASSOC) ?: array()));
    ...
    $stmt->execute($metaParams);
    $auditMetaRows = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
if (table_exists($pdo, 'attribution_runs')) {
    ...
    $auditAttrSummary = array_merge($auditAttrSummary, array_map('intval', $stmt->fetch(PDO::FETCH_ASSOC) ?: array()));

    $stmt = $pdo->prepare("SELECT * FROM attribution_runs WHERE $attrWhereSql ORDER BY id DESC LIMIT 100");
    $stmt->execute($attrParams);
    $auditAttrRows = $stmt->fetchAll(PDO::FETCH_ASSOC);
}
...
<div class="app-shell" data-app-shell>
    ...
    <aside class="app-sidebar" data-sidebar>
        ...
        <nav class="p-3 space-y-1.5 flex-1">
            ...
            <button type="button" class="nav-item group" data-section-target="audit">
                <div class="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-slate-400 group-hover:bg-indigo-600/20 group-hover:text-indigo-400 transition-colors">
    ...
    <main class="app-main">
        ...
        <div class="p-4 md:p-6 max-w-[1700px] mx-auto w-full space-y-6">
            ...
            <div class="app-section hidden" data-app-section="audit">
            <section id="cron-audit" class="glass-card space-y-5">
                <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 pb-4 border-b border-white/10">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            <h2 class="text-xl font-bold text-white">Auditoria do Cron e Integracoes</h2>
                            <p class="text-xs text-slate-400 mt-1">Execucoes automaticas, erros da Meta, linhas processadas e historico tecnico do sistema.</p>
                        </div>
                    </div>
                ...
                <form method="get" action="#cron-audit" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-7 gap-4">
                    ...
                    <div class="form-group">
                        ...
                        <select name="audit_meta_scope" class="pro-select">
                            ...
                            <option value="ad" <?= $auditMetaScope === 'ad' ? 'selected' : '' ?>>Anuncio</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Tipo Atribuicao</label>
                        <select name="audit_attr_type" class="pro-select">
                            <option value="">Todos</option>
                            <option value="import" <?= $auditAttrType === 'import' ? 'selected' : '' ?>>Importacao</option>
                            <option value="match" <?= $auditAttrType === 'match' ? 'selected' : '' ?>>Matching</option>
                            <option value="daily" <?= $auditAttrType === 'daily' ? 'selected' : '' ?>>Consolidacao</option>
                        </select>
                    </div>

                    <div class="form-group">
                        <label class="form-label">Status Atribuicao</label>
                    ...
                    <div class="col-span-full flex flex-wrap items-center justify-end gap-3">
                        <a href="?" class="btn-pro btn-secondary">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3">
                    <div class="p-3 bg-slate-900/60 rounded-xl border border-white/5"><span class="text-slate-400 text-xs block">Meta Execucoes</span><strong class="text-xl font-mono text-white"><?= number_format((int)$auditMetaSummary['total'], 0, ',', '.') ?></strong></div>
                    ...
                    <div class="p-3 bg-slate-900/60 rounded-xl border border-white/5"><span class="text-slate-400 text-xs block">Atr. Execucoes</span><strong class="text-xl font-mono text-white"><?= number_format((int)$auditAttrSummary['total'], 0, ',', '.') ?></strong></div>
                ...
                <div class="space-y-3">
                    <h3 class="text-sm font-bold text-white flex items-center gap-2"><i data-lucide="server-cog" class="w-4 h-4 text-cyan-400"></i><span>Saude das BMs</span></h3>
                    <div class="table-container">
                        <table class="pro-table text-xs">
                            <thead><tr><th>BM</th><th>Status</th><th>Intervalo</th><th>Moeda</th><th>Ultimo Sucesso</th><th>Ultimo Erro</th><th>Mensagem do Erro</th></tr></thead>
                            <tbody>
                ...
                <div class="space-y-3">
                    <h3 class="text-sm font-bold text-white flex items-center gap-2"><i data-lucide="activity" class="w-4 h-4 text-indigo-400"></i><span>Execucoes Meta Graph API</span></h3>
                    <div class="table-container">
                        <table class="pro-table text-xs">
                            <thead><tr><th>ID</th><th>BM</th><th>Escopo</th><th>Periodo</th><th>Inicio</th><th>Duracao</th><th>Status</th><th>Linhas</th><th>Mensagem</th></tr></thead>
                            <tbody>
                            <?php if (!$auditMetaRows): ?>
                                <tr><td colspan="9" class="text-center text-slate-500 py-5">Nenhuma execucao Meta encontrada para os filtros.</td></tr>
                ...
                <div class="space-y-3">
                    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
                        <h3 class="text-sm font-bold text-white flex items-center gap-2"><i data-lucide="git-merge" class="w-4 h-4 text-emerald-400"></i><span>Execucoes de Atribuicao</span></h3>
                        <span class="text-xs text-slate-500">A tabela de atribuicao e geral; ela nao grava BM por execucao.</span>
                    </div>
                    <div class="table-container">
                        <table class="pro-table text-xs">
                            <thead><tr><th>ID</th><th>Tipo</th><th>Inicio</th><th>Duracao</th><th>Status</th><th>Stats</th><th>Mensagem</th></tr></thead>
                            <tbody>
                            <?php if (!$auditAttrRows): ?>
                                <tr><td colspan="7" class="text-center text-slate-500 py-5">Nenhuma execucao de atribuicao encontrada para os filtros.</td></tr>
                ...
                <div class="space-y-3">
                    <h3 class="text-sm font-bold text-white flex items-center gap-2"><i data-lucide="file-warning" class="w-4 h-4 text-amber-400"></i><span>Log Tecnico do Sistema</span></h3>
                    <div class="table-container">
                        <table class="pro-table text-xs">
                            <thead><tr><th>Ultimas linhas de logs/app.log</th></tr></thead>
                            <tbody>
                            <?php if (!$auditAppLogLines): ?>
                                <tr><td class="text-center text-slate-500 py-5">Arquivo de log vazio ou ainda nao criado.</td></tr>
